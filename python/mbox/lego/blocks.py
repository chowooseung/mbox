...
class BlocksUI(QtWidgets.QDialog, blocks_ui.Ui_Form):

    def __init__(self, parent=None):

        super(BlocksUI, self).__init__(parent)
    ...
    def keyPressEvent(self, event):
        ...
        if not event.key() == QtCore.Qt.Key_Escape:
            super(BlocksUI, self).keyPressEvent(event)


class Blocks(MayaQWidgetDockableMixin, QtWidgets.QDialog):

    def __init__(self, parent=None):

        self.toolName = "legoBlockComponentManager"
        super(Blocks, self).__init__(parent=parent)

        self.component_ui = BlocksUI()
...
def show_blocks(*args):
    pyqt.showDialog(Blocks, dockable=True)
